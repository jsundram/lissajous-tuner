...
// The shell as the tests need it. Unlike the skeleton (whose root booted scriptless, making usage/
// the bootability case), this app is one page and app.js owns its content — so the ROOT document is
// the bootability case, and "cached but unbootable" below means root cached with app.js absent.
...
(async () => {
  ...
  reset("slow");
  CACHE.set(BASE, makeResponse("CACHED_ROOT"));   // doc cached, app.js absent → not bootable
  ...
  reset("ok", 500);
  CACHE.set(BASE, makeResponse("CACHED_UNBOOTABLE_ROOT"));   // cached but app.js absent
  r = await start(req(BASE, "navigate"));
