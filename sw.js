...
const V = "lt-v10";     // <-- BUMP ON EVERY SHELL CHANGE (rename the stem freely; keep the digits)
...
function cachePut(req, resp) {
  ...
  // (index.html + styles.css + app.js + theme.js + tuner.js + tuner-worklet.js); a document from
  // one deploy driving scripts from another is exactly the
...
//     EMPTY *only while that placeholder stands*. Once app.js/tuner.js own the content, they and
...
const BOOT_DEPS = {
  ...
  // anything renders, so a missing theme.js throws past the first paint. tuner.js is added in the
...
//   - It HARDCODES the palette (#f4f3ef / #0e1413 / #0f766e) and system-ui — a
...
<style>
  body{margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;
       background:#f4f3ef;color:#16211f;font:16px/1.5 system-ui,-apple-system,sans-serif}
  ...
  button{margin-top:1.2em;border:0;border-radius:999px;padding:.7em 1.3em;
         background:#0f766e;color:#fff;font-size:1em;cursor:pointer}
  @media (prefers-color-scheme:dark){ body{background:#0e1413;color:#e8efed} p{color:#aaa} }
</style>
<main>
